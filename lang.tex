...
\subsection{Syntax}
The syntax of trace expressions $\re$ is defined by the following grammar, parametric in a given universe of constraints $\cset$. We assume fixed countably infinite sets of predicate symbols $\predSet$, of values $\vset$, and of variables $\varSet$.
\begin{center}
 %  \begin{small}
 \begin{grammar}
...
We omit the standard definitions of free variables $\fv(\re)$, $\fv(\atom)$, and $\fv(\cts)$,
and the corresponding definition of variable substitution $\subs{\re}{\var}{\val}$, $\subs{\atom}{\var}{\val}$ and $\subs{\cts}{\var}{\val}$. These definitions assume that the considered notion of constraint is equipped with the corresponding definition of set of variables occurring in a constraint, and of substitution of a variable with a value in a constraint, \st $\fv(\subs{\ct}{\var}{\val})=\fv(\ct)\setminus\{\var\}$, and $\subs{\ct}{\var}{\val}=\ct$ if $\var\notin\fv(\ct)$, for all $\ct\in\cset$, $\var\in\varSet$ and $\val\in\vset$.
...
\begin{definition}\label[definition]{def:langOp}
 ...
 \]
\end{definition}
The following definition introduces the denotational semantics of trace expressions.
It is parametric in the following two functions $\match$ and $\sol$:
the first is a \emph{partial} function associating sets of events with atoms, while the second is a total three-valued function modeling constraint solving \cite{JAFFAR1998}.
More precisely, $\match(\ev,\atom)$ is defined only iff $\ev$ is an event defined by $\atom$;
in this case, the function returns the set of constraints $\cts$, with $\fv(\cts)\subseteq\fv(\atom)$, that must be satisfied for the atom to be considered as matching the event.
If $\sol(\cts)=\true$, then $\cts$ is satisfiable, while it is unsatisfiable if $\sol(\cts)=\false$; finally, if $\sol(\cts)=\unknown$, then it is unknown whether $\cts$ is satisfiable or not.

The function $\sol$ is assumed to \emph{agree} with the underlying constraint theory $\theory$.
Let $\cts=\{\overline{\ct}\}$, with $\fv(\cts)=\{\overline{\var}\}$;
if $\sol(\cts)=\true$ then $\theory\models\overline{\exists\var.\ct}$, and
if $\sol(\cts)=\false$ then $\theory\models\neg\overline{\exists\var.\ct}$.
The following conditions are assumed to hold: if $\match(\ev,\atom)=\cts$, then $\sol(\cts)\neq\false$, and if $\sol(\cts)=\false$ then $\sol(\subs{\cts}{\var}{\val})=\false$.

\begin{definition}\label[definition]{def:sem}
 Let $\vset_{\cts,\var}$ denote the set of values $\val$ s.t. $\sol(\subs{\cts}{\var}{\val})\neq\false$.

 The denotational semantics of trace expressions is defined by induction as follows:
 \[
  \begin{array}{l}
   \sem{\none}=\emptyset\quad \sem{\eps}=\{\emptyTrace\} \quad \sem{\predSym(\overline{\val})}=\{\ev \mid \match(\ev,\predSym(\overline{\val}))\equiv\emptyset\} \\
   ...
   \sem{\existOp{\var}{\cts}{\re}}=\bigcup_{\val\in\vset_{\cts,\var}}\sem{\subs{\re}{\var}{\val}}
  \end{array}
 \]
\end{definition}

Note that in \cref{def:sem}, the semantics of an atom ($\atom=\predSym(\overline{\val})$) is defined only if it is ground; this implies that the semantics of open  trace expressions is undefined, although they are considered syntactically correct.
...
\begin{figure*}[t]
 $$
  \begin{array}{c}
   ...
   \Rule{at}{}{\atom\parDer{\ev,\cts}\eps}{\match(\ev,\atom)=\cts}                                          \\[4ex]
...
\begin{tabular}{l}
 $\match(\ev,\pongPr(\eventTerm))=\{\eventTerm = n\}$ iff \\ \qquad $\ev=\pongEv(n)$ and $\sol(\{\eventTerm = n\})\neq\false$ \\
 $\match(\ev,\pingPr(\eventTerm))=\{\eventTerm = n\}$ iff \\ \qquad$\ev=\pingEv(n)$ and $\sol(\{\eventTerm = n\})\neq\false$
\end{tabular}
